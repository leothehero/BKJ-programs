...
if args[1] == "ReactorAccess" then
    ...
    if success then
        ...
        io.stderr:write("Could not close, reason: ".. tostring(reason))
    end
    ...
    if not(event) then
        ...
        if AuthUsers[UUID] then
            ...
            for k,v in pairs(ViewDoorAddresses) do
                local success,reason = component.proxy(k).close(args[2])
                if success then
                    print("Closing "..k)
                else
                    io.stderr:write("Could not close "..k..", reason: ".. tostring(reason))
                end
            end
        --else
